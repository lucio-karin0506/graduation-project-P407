...
class Simulator:
    ...
    def read_file(self, file_name:str) -> None:
        ...
        None.
        """
        file = pathlib.Path(os.getcwd()+'/orderFile/'+file_name)
    ...
    def buy(self, br):        
        ...
        if br.order_option == 'all':
            ...
            volcount = int(self._cash / int(br.order_value))
    ...
    def sell(self, sr):        
        ...
        if sr.order_option == 'all':            
            if len(self._stu_log[self._stu_log['item_code']==sr.item_code]['item_count']) == 0:
                self.print_error(2)
                return
        ...
        if sr.order_option == 'por':            
            ...
            volcount\
                = int(self._stu_log[self._stu_log['item_code']==sr.item_code]['item_count']) * float(sr.order_value)
        ...
        if sr.order_option == 'scnt':            
            if len(self._stu_log[self._stu_log['item_code']==sr.item_code]['item_count']) == 0 or \
    ...
    def write_trlog(self, tpath='cwd'):        
        '''
        ...
        if tpath == 'cwd':
            tpath = os.getcwd()
            os.makedirs(tpath+'/tradingLogFile', exist_ok=True)     

        tdict = self._trd_log.to_dict(orient='records')
        #json.dumps(tdict, ensure_ascii=False, indent='\t')
        with open(tpath+'/tradingLogFile/'+self.stock_name[0]+'_'+self.stock_name[1]+'_TradingLog.json', 'w+', encoding='utf-8') as make_file:
            json.dump(tdict, make_file, ensure_ascii=False, indent='\t')
    
    def write_stlog(self, spath='cwd'):        
        '''
        Write Status.json
        ...
        if spath == 'cwd':
            spath = os.getcwd()
            os.makedirs(os.getcwd()+'/statusFile', exist_ok=True)        
        
        sdict = self._stu_log.to_dict(orient='records')
        sdict = {'cash' : self._cash, 'stock' : sdict}
        #json.dumps(tdict, ensure_ascii=False, indent='\t')
        with open(spath+'/statusFile/'+self.stock_name[0]+'_'+self.stock_name[1]+'_Status.json', 'w+', encoding='utf-8') as make_file:
    ...
    def get_Tcount(self):
        ...
        None.          
        '''
        return self._stu_log.loc[1, 'buy_count'], self._stu_log.loc[1, 'sell_count']
    ...
    def print_error(self, erc):
        ...
        error code 5 -> order file이 존재하지 않음
        error code
        error code
        '''
...
if __name__ == '__main__':
    ...
    mod.read_file('SK하이닉스_d_Order.json')
    
    # 매수수수료, 매도수수료, 국가세금, 슬리피지 허용범위, 네트워크사용 유무
    mod.set_option(0.015, 0.015, 0.23, 0.01, False)
