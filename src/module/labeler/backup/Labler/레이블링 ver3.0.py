...
def set_RRPB(df, local_max, local_min, RRPB, label_target):
    ...
    while (True):
        ...
        if ((i + 1 >= len(local_min)) or (j + 1 >= len(local_max))):  ### 모든 local_minmax에 대해 레이블링 완료하면 종료
            break
        elif (local_min[i + 1] > local_max[j + 1]):  ### 다음 local_min이 다음 local_max보다 클때 -> local_min 유지
            j += 1
        elif (local_min[i + 1] < local_max[j]):  ### 다음 local_min이 현재 local_max보다 작을 때 -> local_max 유지
            i += 1
        else:
            i += 1
            j += 1
    ...
    for i in range(1, bottom_len):
        ...
        for k in range(len(top_index)):
            if (top_index[k] > bottom_index[i - 1]) and (top_index[k] < bottom_index[i]):
                # bottom <= (1-RFPT)을 만족하지 못하는 top, bottom을 top,bottom컬럼에서 제거
    ...
    for curr in tb_list:
        ...
        if (tb_flag == 1):
            ...
            for prev in range(curr, -1, -1):
                ...
                if (df.loc[prev, label_target] >= df.loc[curr, label_target] * (1 - TBR)) and (
                        df.loc[prev, label_target] <= df.loc[curr, label_target]):
                    df.loc[prev, label_target + '_top_zone'] = 1
                else:
                    break
            # 연속된 이후 지점들과 비교
            for after in range(curr + 1, len(df)):
                if (df.loc[after, label_target] >= df.loc[curr, label_target] * (1 - TBR)) and (
                        df.loc[after, label_target] <= df.loc[curr, label_target]):
                    df.loc[after, label_target + '_top_zone'] = 1
                else:
                    break
        elif (tb_flag == -1):
            for prev in range(curr, -1, -1):
                if (df.loc[prev, label_target] <= df.loc[curr, label_target] * (1 + BBR)) and (
                        df.loc[prev, label_target] >= df.loc[curr, label_target]):
                    df.loc[prev, label_target + '_bottom_zone'] = 1
                else:
                    break
            for after in range(curr + 1, len(df)):
                if (df.loc[after, label_target] <= df.loc[curr, label_target] * (1 + BBR)) and (
                        df.loc[after, label_target] >= df.loc[curr, label_target]):
...
if __name__ == '__main__':
    ...
    # 직접 CSV파일 불러오기
    # df = pd.read_csv("./test/csv/SPL1.csv", index_col='Date', encoding='CP949', parse_dates=['Date'])
