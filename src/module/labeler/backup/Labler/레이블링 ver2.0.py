...
2. local_max(고점)이 시작이거나, local_min(저점)이 종료일 경우에 해당하는 index를 제거한다.
...
def set_point(df, RRPB, RFPT, TBR, BBR, label_target):
    ...
    # 차트에서 고점이 start인 경우 저점이 start로 change
    max_len = len(local_max)
    for _ in range(max_len):
        if (local_max[0] < local_min[0]):
            local_max = np.delete(local_max, 0, 0)
        else:
            break

    # 차트에서 저점이 finish인 경우 고점이 finish로 change
    min_len = len(local_min)
    for _ in range(min_len):
        if (local_max[-1] < local_min[-1]):
            local_min = np.delete(local_min, len(local_min) - 1, 0)
        else:
            break
...
3. (직전 bottom < top < 현재 bottom)인 top 과 현재 bottom을 비교
4. (top - bottom >= top * RFPT) 을 만족하지 못하는 top과 bottom을 top,bottom column에서 제거
5. 데이터프레임 리턴
'''
    ...
    for i in range(1, len(bottom_index)):
        ...
        for k in range(len(top_index)):
            if (top_index[k] > bottom_index[i - 1]) and (top_index[k] < bottom_index[i]):
                # break
                # top - bottom <= top * RFPT
                if df.loc[top_index[k], label_target] - df.loc[bottom_index[i], label_target] <= df.loc[
                    top_index[k], label_target] * RFPT:
...
# 차트 시각화
def visual_charts(df, rrpb, rfpt):
    df['Close'].plot()
    plt.scatter(df[df['Close_top'] == 1].index, df[df['Close_top'] == 1]['Close'], c='r', label='top')
    plt.scatter(df[df['Close_bottom'] == 1].index, df[df['Close_bottom'] == 1]['Close'], c='g', label='bottom')
    plt.title(f"RRPB: {rrpb}, RFPT: {rfpt}")
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    df = make_label(config['code'],
                    ...
                    config['TBA'],
                    config['BBR'])
    #visual_charts(df, config['RRPB'], config['RFPT'])
