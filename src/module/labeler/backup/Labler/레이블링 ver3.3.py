...
def make_label(df, stock_name, dtype, label_target_list, RRPB, RFPT, TBR, BBR):
    ...
    if(dtype == 'D'):
        ...
        createFolder(f"./label_data/label_weekly/{name}_label_weekly")
...
def set_RRPB(df, RRPB, label_target):
    ...
    for i in range(len(local_max)):
        ...
        temp_df = df[:local_max[i]] 
        prev_min = max(temp_df[temp_df['local_min'] == 1].index)
        for j in range(len(local_min)):
            ...
            if(prev_min < local_max[0]):
                ...
                temp_df1 = df[:prev_min]   
                prev_max = max(temp_df1[temp_df1['local_max'] == 1].index)
