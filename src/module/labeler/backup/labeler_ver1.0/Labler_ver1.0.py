...
def make_label(df, stock_name, dtype, label_target_list, RRPB, RFPT, TBR, BBR):
    ...
    # 저장  
    if(dtype == 'D'):
        createFolder(f"./label_data/label_daily/{stock_name}_label_daily")
        df.to_csv(f'./label_data/label_daily/{stock_name}_label_daily/{stock_name}_{RRPB}_{RFPT}_{TBR}_{BBR}_{dtype}.csv')
    else:
        createFolder(f"./label_data/label_weekly/{stock_name}_label_weekly")
        df.to_csv(f'./label_data/label_weekly/{stock_name}_label_weekly/{stock_name}_{RRPB}_{RFPT}_{TBR}_{BBR}_{dtype}.csv')
...
def set_point(df, RRPB, RFPT, TBR, BBR, label_target):  
    ...
    # top, bottom이 없을 경우
    if sum(df[label_target+'_top']) == 0 or sum(df[label_target + '_bottom']) == 0:
...
def set_RRPB(df, RRPB, label_target):
    ...
    for i in range(len(local_max)):
        # 현재 local_max 직전 local_min
        flag = 0
        for k in range(len(local_min)):
            if local_min[k] > local_max[i]:
                prev_min = local_min[k-1]
                flag = 1
                break
        if(flag == 0):
            prev_min = local_min[-1]
            
        
            # start_point < local_min[j] < local_max[0]
        if(prev_min < local_max[0]):
            for j in range(len(local_min)):
                if(local_min[j] < local_max[i]):
                    ...
                    elif(df.loc[local_min[j], label_target] == df.loc[local_max[i], label_target]):
                        df.loc[local_max[i], top_column] = 1
                        df.loc[local_min[j], top_column] = 1
                else:
                    break
        else:
            # prev_max : local_max[i] 직전 local_min의 직전 local_max
            # prev_max < local_min[j] < local_max[i]
            # local_max나 local_min이 연속된 지점들을 정제하기 위함
            for k in range(len(local_max)):
                if local_max[k] > prev_min:
                    prev_max = local_max[k-1]
                    break
            for j in range(len(local_min)):
                if(local_min[j] > prev_max and local_min[j] < local_max[i]):
                    if(df.loc[local_max[i], label_target] >= df.loc[local_min[j], label_target] * (1 + RRPB)):
                        df.loc[local_max[i], top_column] = 1
                        df.loc[local_min[j], bottom_column] = 1
                    # 마지막 지점들
                    if((i == len(local_max) - 1) and (df.loc[local_max[i], label_target] == df.loc[local_min[j], label_target])):
                        df.loc[local_max[i], bottom_column] = 1
                        df.loc[local_min[j], bottom_column] = 1

                # curr local_max보다 큰 경우 break
                if(local_min[j] > local_max[i]):
                    break
...
if __name__ == '__main__':
    ...
    # 직접 CSV파일 불러와서 config파일에 있는 파라미터 값으로 레이블링
    # df = pd.read_csv("./test/csv/input04.csv", index_col='Date', encoding='CP949', parse_dates=['Date'])
    # row = config[0]
    ...
    # config파일을 통하여 주가차트 및 파라미터 불러온 후 레이블링
    # for row in config:    
    #     
