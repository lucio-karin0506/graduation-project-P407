...
class Calculator:
    ...
    def read_file(self, file_name:str) -> None:
        ...
        None.
        """
        file = pathlib.Path(os.getcwd()+'/tradingLogFile/'+file_name)
    ...
    def write_atlog(self, nopt, apath='cwd'):        
        '''
        Parameters
        ----------
        apath : str
        ...
        if apath == 'cwd':
            apath = os.getcwd()
            os.makedirs(apath+'/Asset'+nopt+'File', exist_ok=True)
        ...
        with open(apath+'/Asset'+nopt+'File/'+self.stock_name[0]+'_'+self.stock_name[1]+'_Asset'+nopt+'.json', 'w+', encoding='utf-8') as make_file:
    ...
    def get_Wasset(self):
        ...
        None.
        """
        return self._wast_log['cumulative'].iloc[-1],\
                max(self._wast_log['cumulative']),\
                min(self._wast_log['cumulative'])
    ...
    def get_Masset(self):
        ...
        None.
        """
        return self._mast_log['cumulative'].iloc[-1],\
                max(self._mast_log['cumulative']),\
                min(self._mast_log['cumulative'])
    ...
    def get_Yasset(self):
        ...
        None.
        """
        return self._yast_log['cumulative'].iloc[-1],\
                max(self._yast_log['cumulative']),\
                min(self._yast_log['cumulative'])
    ...
    def _print_error(self, erc):
        ...
        error code 1 -> tradingLog file이 존재하지 않음
        error code
        error code
        """        
        error = {1 : '주문파일이 존재하지 않습니다.'}
...
if __name__ == '__main__':
    ...
    file = pathlib.Path(os.getcwd()+'/TradingLogFile/'+'TradingLog.json')
    text = file.read_text(encoding='utf-8')
    js = json.loads(text)
    trade = pd.DataFrame(js)
