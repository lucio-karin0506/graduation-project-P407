...
class Upbit_Api():
    ...
    def error_handler(self, res):
        if res.status_code == 400:
            print('<WARNING> message:', res.json()['error']['message'], ', name:', res.json()['error']['name'])
            return 0
    ...
    def get_candle_min(self, market: str = 'KRW-BTC', unit: str = '1', to: str = '', count: str = '200') -> pd.DataFrame:
        ...
        if self.error_handler(response) == 1:
            ...
            df.rename(columns = {'opening_price': 'open',
                                 'high_price': 'high',
                                 'low_price': 'low',
                                 'trade_price': 'close'}, inplace = True)
            df = df.sort_values('timestamp').reset_index().drop('index', axis='columns')
            return df
    ...
    def get_candle(self, market:str, start_date:str='', end_date:str='', limit:int=100, interval:str='30m'):
    ...
    def cancel_orders(self, market:str, side:str):
        """
        ...
        if len(orders) != 0:            
            wait_uuids =list(orders[orders['side']==side]['uuid'])
            if len(wait_uuids) != 0:
