...
return: boolean    '''    
def crossUp(row, idx:int, df, source:str, target:str or int) -> bool:
    ''' target이 정수일 때와 문자열(df의 column)일 때를 구분해야 함 '''
...
return: boolean    ''' 
def crossDown(row, idx:int, df, source:str, target:str or int) -> bool:
    ''' target이 정수일 때와 문자열(df의 column)일 때를 구분해야 함 '''
...
# def get_candletype(self, df, up_pct, down_pct):       
