...
from module.order_creator.indicator import *
from module.order_creator.checker import *
from module.labler.labler import *
...
class Request:
    ...
    def __init__(self, mode):
        ...
        mode: {'local', 'network'}
            주문생성 모드
            'local'일 땐 Request 객체에 필요한 데이터를 생성할 때 local file로부터 생성한다.
            'network'일 땐 network를 이용하여 생성한다.
    ...
    def _get_stock(self) -> None:
        ...
        mode 멤버변수를 확인하여 local-> local file에서 주가데이터를 생성함, network-> FinanceDataReader로 주가데이터를 생성함
...
class OrderCreator:
    ...
    def read_file(self, file_name:str) -> None:
        ...
        for i in df.index:
            self.order_requests['strategy_'+str(i)] = Request(self.mode)
    ...
    def make_order(self) -> None:
        ...
        def make_strategy(strategy:str) -> str:
            ...
            string.
            """           
            strategy = strategy.replace('sell(', 'self._sell(trade, inner_idx, request.stock_df, ').replace('buy(', 'self._buy(trade, inner_idx, request.stock_df, ')

            if strategy in 'crossDown' or 'crossUp':
                strategy = strategy.replace('crossDown(', 'crossDown(trade, inner_idx, request.stock_df, ').replace('crossUp(', 'crossUp(trade, inner_idx, request.stock_df, ')
        ...
        for request in self.order_requests.values():
