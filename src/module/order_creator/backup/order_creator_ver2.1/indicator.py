...
def add_RSI(df: "DataFrame", period=14, target='close') -> "DataFrame":
    """
...
def add_MA(df: "DataFrame", period=10, target='close') -> "DataFrame":
    """
...
def add_EMA(df: "DataFrame", period=30, target='close') -> "DataFrame":
    """
...
def add_CMO(df: "DataFrame", period=14, target='close') -> "DataFrame":
    """
...
if __name__ == "__main__":
    ...
    # local mode
    df = pd.read_csv(os.getcwd()+'/stockFile/'+'삼성전자_d.csv', index_col='Date')
