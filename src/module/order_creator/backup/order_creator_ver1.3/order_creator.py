...
class OrderCreator(gathering.Gathering):
    ...
    def extract(self, df):
        ...
        self._indicator = list(df['indicator'])
        ...
        for idx, tech_value in enumerate(self._indicator):
            ...
            self._parameters.append([str(to_int(x)) for x in paramitor_list if to_int(x) is not None]) # 문자열중에서 int로 변환가능한 것을 변환한다.
            ...
            if self._datetype[idx] == 'D' or self._datetype[idx] == 'd':
                self._tech_date[idx] = self._tech_date[idx] - relativedelta(days=max(preiod_list)+30)
            
            elif self._datetype[idx] == 'W' or self._datetype[idx] == 'w':
                self._tech_date[idx] = self._tech_date[idx] - relativedelta(weeks=max(preiod_list)+30)   
    ...
    2021.01.20: mode 파라미터추가 local-> local file에서 주가데이터를 생성함, network-> fdr로 주가데이터를 생성함
    func: 주가데이터프레임을 생성하고 해당 주가의 코드와 이름을 데이터프레임에 추가한다.
    parameter: mode
    return: 없음
    '''
    def _get_stock(self, mode='local'):
        self._df_list=[]
        ...
        for i in range(len(self._stockcode)):  
            if mode == 'local':
                ...
                if not os.path.isfile(os.getcwd()+'/stockFile/'+name+'_'+self._datetype[i]+'.csv'):
                    self.print_error(3)                    
                
                df = pd.read_csv(os.getcwd()+'/stockFile/'+name+'_'+self._datetype[i]+'.csv', index_col='Date')
    ...
    def _add_indicator(self):
        for i in range(len(self._indicator)):
            for j in range(len(self._indicator[i])):
                tech_df = eval(self._indicator[i][j].replace('(', '(self._df_list[i],'))
    ...
    2021.01.20: mode파라미터 추가 local, network
    func: 전략이 만족할 때 buy(sell)함수를 호출하고 그 정보를 json파일로 생성한다.
    parameter: mode
    return: 없음'''
    def make_order(self, mode='local'):
        ...
        try:
            self._get_stock(mode=mode) # 주가데이터 생성
        ...
        for idx, trade_df in enumerate(self._df_list):
            ...
            if mode == 'network': # fdr로 가져올 때만 변환시켜줌
        ...
        # ------------------------------------------------------------------------------------------------------------------------
            ...
            if self._datetype[idx].upper() == 'D':                
                ...
                with open(os.getcwd()+'/orderFile/'+self._df_list[idx]['item_name'][0]+'_d.json', 'w+', encoding='utf-8') as make_file:
                    json.dump(adict, make_file, ensure_ascii=False, indent='\t')           

            elif self._datetype[idx].upper() == 'W':
                os.makedirs(os.getcwd()+'/orderFile', exist_ok=True)
                with open(os.getcwd()+'/orderFile/'+self._df_list[idx]['item_name'][0]+'_w.json', 'w+', encoding='utf-8') as make_file:
                    json.dump(adict, make_file, ensure_ascii=False, indent='\t')
...
if __name__ == "__main__":
    ...
    cwd = os.getcwd()
    file = pathlib.Path(cwd+'/strategyFile/DS_strategy_ver2_1.json')
