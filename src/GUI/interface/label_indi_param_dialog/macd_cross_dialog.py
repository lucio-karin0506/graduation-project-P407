...
class macd_cross_label_Param(QDialog):
    def __init__(self, title, path, parent):
        ...
        self.win_size_label = QLabel('윈도우사이즈', self)
        self.win_size_edit = QLineEdit()
        self.win_size_edit.setPlaceholderText('0')
        ...
        param_layout.addRow(self.signal_period_label, self.signal_period_edit)
        param_layout.addRow(self.win_size_label, self.win_size_edit)
    ...
    def confirmIt(self):
        ...
        if self.short_period_edit.text() == '' or self.long_period_edit.text() == '' or self.signal_period_edit.text() == '' or self.win_size_edit.text() == '':
            ...
            gathering_info = {
                                ...
                                'win_size': int(self.win_size_edit.text()),
                                'target': str(self.target_option.currentData())
                            }

            label_indicator.add_macd_cross(gathering_info['df'], gathering_info['short'], gathering_info['long'],
                                    gathering_info['signal'], gathering_info['win_size'], gathering_info['target'])
